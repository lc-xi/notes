...
- **1.创建pc-replicaset.yaml文件，内容如下：**
```yaml
apiVersion: apps/v1
kind: ReplicaSet   
metadata:
  name: pc-replicaset #ReplicaSet控制器的名称
  namespace: dev #ReplicaSet控制器的命名空间
spec:
  replicas: 3 #副本数量 也就是说创建多少个pod
  selector: 
    matchLabels:
      app: nginx-pod #绑定label key位app，value位nginx-pod的pod
  template:
    metadata:
      labels:
        app: nginx-pod
...
- **2.使用pc-replicaset.yaml创建ReplicaSet控制器：**
```shell
# 创建rs
...
- **3.扩缩容：**
```shell
# 方式一：编辑rs的副本数量，修改spec:replicas: 6即可
...
# 这种方式删除ReplicaSet控制器，Pod没有删除，Pod就无法进行扩缩容和镜像升级操作，Pod由Kubernetes直接管理
...
- **1.创建pc-deployment.yaml，内容如下：**
```yaml
apiVersion: apps/v1
kind: Deployment      
metadata:
  name: pc-deployment #Deployment控制器名称
  namespace: dev #命名空间
spec: 
  replicas: 3 
  selector:
...
- **2.使用pc-deployment.yaml创建Deployment控制器：**
```shell
# 创建deployment
# --record=true 开启记录
...
# 方式2：编辑deployment的副本数量，修改spec:replicas: 4即可
...
spec:
  ...
  targetCPUUtilizationPercentage: 3 # CPU使用率指标 3为：3%
